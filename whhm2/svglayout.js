...
class SVGLayoutGenerator {
    constructor() {
        ...
        this.columnRangeFilter = { min: 0, max: 100 }; // Column range filter
        this.horizontalScrollOffset = 0; // Horizontal scroll offset
    ...
    generateSVGLayout(options = {}, heatmapData = null) {
        ...
        const totalWidth = Object.values(colWidths).reduce((sum, width) => sum + width * scaleFactor, 0);
        const scrollOffset = this.horizontalScrollOffset * totalWidth;
        
        let xPos = padding - scrollOffset;
        for (let c = range.s.c; c <= range.e.c; c++) {
            colPositions[c] = xPos;
        ...
        this.currentSheetData.cells.forEach(cell => {
            ...
            const colPosition = (cell.col - range.s.c) / totalCols;
            if (this.columnRangeFilter.max < 1.0 && colPosition > this.columnRangeFilter.max) {
                return; // Skip this cell if it's outside the column range
            }
    ...
    generateSVGWithHeatmap_OLD(heatmapData, options = {}) {
        if (!this.currentSheetData) return null;
        ...
        let svg = `<svg width="${svgWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg">
            <style>
                ...
                /* Heatmap colors */
                .heat-0 { fill: #f1f3f4; } /* No activity */
                .heat-1 { fill: #fef7e0; } /* Very low */
                .heat-2 { fill: #fef3c7; } /* Low */
                .heat-3 { fill: #fde68a; } /* Low-medium */
                .heat-4 { fill: #fcd34d; } /* Medium */
                .heat-5 { fill: #f59e0b; } /* Medium-high */
                .heat-6 { fill: #d97706; } /* High */
                .heat-7 { fill: #b45309; } /* Very high */
                .heat-8 { fill: #92400e; } /* Extremely high */
                .heat-9 { fill: #78350f; } /* Maximum */
            </style>
            
            <!-- Background -->
            <rect width="${svgWidth}" height="${svgHeight}" fill="#ffffff"/>
            
            ${showTitle ? `<text x="${svgWidth/2}" y="25" class="title">Warehouse Activity Heatmap - ${this.currentSheetData.name}</text>` : ''}
        ...
        // Create cells and merged regions with heatmap colors
        this.currentSheetData.cells.forEach(cell => {
            // Apply column range filter
            const totalCols = range.e.c - range.s.c + 1;
            const colPosition = (cell.col - range.s.c) / totalCols;
            ...
            // Get heatmap data for this cell
            const activity = heatmapData.get(cell.text) || 0;
            let heatLevel = 0;
            if (activity > 0) {
                if (activity <= 5) heatLevel = 1;
                else if (activity <= 10) heatLevel = 2;
                else if (activity <= 15) heatLevel = 3;
                else if (activity <= 25) heatLevel = 4;
                else if (activity <= 35) heatLevel = 5;
                else if (activity <= 50) heatLevel = 6;
                else if (activity <= 75) heatLevel = 7;
                else if (activity <= 100) heatLevel = 8;
                else heatLevel = 9;
            }
            ...
            if (cell.mergeInfo && cell.mergeInfo.isStart) {
                ...
                if (!processedMerges.has(mergeKey)) {
                    ...
                    // Draw merged cell rectangle with heatmap color
                    svg += `<rect x="${x}" y="${y}" width="${mergeWidth}" height="${mergeHeight}" 
                             class="merged-cell warehouse-cell heat-${heatLevel}" 
                             data-location="${cell.text}" data-row="${cell.row}" data-col="${cell.col}" 
                ...
                if (!cell.mergeInfo) {
                    ...
                    svg += `<rect x="${x}" y="${y}" width="${width}" height="${height}" 
                             class="cell-border warehouse-cell heat-${heatLevel}" 
                             data-location="${cell.text}" data-row="${cell.row}" data-col="${cell.col}" 
                             data-activity="${activity}"/>`;
                    
                    // Add text with wrapping (same as original layout)
                    if (cell.text && cell.text.trim()) {
                        const fontSize = Math.max(8, 10 * scaleFactor);
                        const lines = this.wrapText(cell.text, width, fontSize);
                        const lineHeight = fontSize * 1.2;
    ...
    setHorizontalScroll(value, maxValue) {
        if (value === 0) {
            this.horizontalScrollOffset = 0;
        } else {
            const percentage = value / maxValue;
            this.horizontalScrollOffset = percentage;
        }
        console.log('Horizontal scroll offset:', this.horizontalScrollOffset);
    }

    // Debug method to show which columns are being processed
    debugColumnRange() {
        if (!this.currentSheetData) {
            console.log('No sheet data loaded');
            return;
        }
        ...
        console.log('Columns being processed:', cols.join(', '));
        console.log('Total columns:', cols.length);
        console.log('First column:', cols[0]);
        console.log('Last column:', cols[cols.length - 1]);
        
        // Check if 'W' column is included
        const wColIndex = XLSX.utils.decode_col('W');
        const isWIncluded = wColIndex >= range.s.c && wColIndex <= range.e.c;
        console.log('Is column W included?', isWIncluded);
        
        return {
            columns: cols,
            totalColumns: cols.length,
            firstColumn: cols[0],
            lastColumn: cols[cols.length - 1],
            includesW: isWIncluded
        };
    }
    ...
    // Method to check if using SVG layout (for compatibility)
    isUsingSVGLayout() {
        return true; // This class always uses SVG layout
    }

    // Method to get the SVG layout generator (for compatibility)
    getSVGLayoutGenerator() {
        return this;
    }

    // Method to handle layout upload (for compatibility with app.js)
    handleLayoutUpload(event) {
        return this.processExcelFile(event.target.files[0]);
    }

    // Method to handle shipping upload (for compatibility with app.js)
    handleShippingUpload(event) {
        // This method is not implemented in SVGLayoutGenerator
        // It should be handled by the main app
        console.log('Shipping upload should be handled by the main app');
    }

    // Property to store shipping data (for compatibility)
    get shippingData() {
    ...
    // Property to store layout data (for compatibility)
    get layoutData() {
        return this.convertToLayoutData();
    }
}
