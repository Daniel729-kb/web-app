        ...
        // 以下、既存の関数群で158をmaxHeightLimitまたはgetMaxCartonHeight()に置換
        ...
        function updateCombinePreview() {
                const originalLimit = maxHeightLimit;
                try {
                    maxHeightLimit = Number.POSITIVE_INFINITY;                    
                                ...
                                let bestPalletSize = null;
                                
                                selectedPalletSizes.forEach(palletSize => {
                                    const itemCounts = combinedCartons.reduce((acc, carton) => {
                                        if (!acc[carton.code]) {
                                            acc[carton.code] = {
                                                code: carton.code,
                                                count: 0,
                                    ...
                                    items.forEach(item => {
                                        ...
                                        const rotatedFits = Math.floor(palletSize.width / item.w) * Math.floor(palletSize.depth / item.l);
                                        const maxPerLayer = Math.max(normalFits, rotatedFits);
                                        const requiredLayers = Math.ceil(item.count / maxPerLayer);
                                        const itemHeight = requiredLayers * item.h;
                                    ...
                                    console.log(`${palletSize.name}: 推定高さ${estimatedHeight.toFixed(1)}cm, 制限内: ${estimatedHeight <= maxHeightLimit}`);
                                    
                                    if (canPlaceAll && estimatedHeight <= maxHeightLimit) {
                                        if (!bestEstimate || estimatedHeight < bestEstimate.height) {
                                            bestEstimate = {
                                                height: estimatedHeight,
                                                feasible: true
                                            };
                                            bestPalletSize = palletSize;
                                        }
                                    }
                                });
                                ...
                                let displayHeight, canCombine, statusMessage;
                                
                                if (bestEstimate) {
                                    displayHeight = bestEstimate.height;
                                    canCombine = bestEstimate.feasible;
                                    statusMessage = canCombine ? 
                                        `✅ 結合可能 (${bestPalletSize.name}, 制限${maxHeightLimit}cm以内)` : 
                                        '❌ 結合困難';
                                } else {
                                    displayHeight = 999;
                                    canCombine = false;
                                    statusMessage = `❌ 高さ制限${maxHeightLimit}cm超過のため結合不可`;
                                }
                                
                                previewDiv.innerHTML = `
                                    <strong>結合プレビュー (高さ制限: ${originalLimit}cm):</strong><br>
                                    📦 総カートン数: ${totalCartons}個<br>
                                    ⚖️ 総重量: ${totalWeight.toFixed(1)}kg<br>
                                    📏 推定高さ: ${displayHeight.toFixed(1)}cm<br>
                                    🏷️ 貨物種類: ${allCodes.length}種類 (${allCodes.join(', ')})<br>
                                    ${statusMessage}<br>
                                    <small style="color: #666;">
                                        ※ 高さ制限${maxHeightLimit}cmを考慮した計算結果
                                    </small>
                                `;
                            
                } finally {
                    maxHeightLimit = originalLimit;
                }}
        ...
        function combinePallets() {
                ...
                try {
                    maxHeightLimit = Number.POSITIVE_INFINITY;                    
                                ...
                                console.log(`\n=== パレット結合（高さ制限: ${maxHeightLimit}cm） ===`);
                                
                                const combinedCartons = [...pallet1.cartons, ...pallet2.cartons];
                                
                                let bestCombinedPallet = null;
                                let bestScore = 0;
                                
                                for (const palletSize of selectedPalletSizes) {
                                    const testPallet = calculateCombinedPalletConfiguration(combinedCartons, palletSize);
                                    if (testPallet && testPallet.height <= maxHeightLimit) {
                                        const score = calculatePalletScore(testPallet, []);
                                        console.log(`${palletSize.name}: 高さ${testPallet.height.toFixed(1)}cm ≤ 制限${maxHeightLimit}cm, スコア${score.toFixed(1)}`);
                                        
                                        if (score > bestScore) {
                                            bestScore = score;
                                            bestCombinedPallet = testPallet;
                                        }
                                    } else if (testPallet) {
                                        console.log(`${palletSize.name}: 高さ制限超過 ${testPallet.height.toFixed(1)}cm > ${maxHeightLimit}cm`);
                                    }
                                }
                                
                                if (!bestCombinedPallet) {
                                    alert(`⚠️ パレット結合失敗\n\n設定された高さ制限${maxHeightLimit}cm以内での結合ができませんでした。\n\n解決方法:\n• 高さ制限を緩和\n• より大きなパレットサイズを選択\n• カートン数を調整`);
                                    return;
                                }
                                ...
                                window.currentPallets.push(bestCombinedPallet);
                                
                                console.log(`✅ 結合完了: 高さ${bestCombinedPallet.height.toFixed(1)}cm ≤ 制限${maxHeightLimit}cm`);
                    ...
                    displayResults(window.currentPallets);
                                ...
                                const successDiv = document.createElement('div');
                                successDiv.className = 'alert alert-success';
                                successDiv.innerHTML = `🔄 パレット結合成功！<br>
                                    📏 高さ: ${bestCombinedPallet.height.toFixed(1)}cm ≤ 制限${maxHeightLimit}cm<br>
                                    🚛 パレット数削減: 2枚 → 1枚`;
                                document.getElementById('errors').appendChild(successDiv);
        ...
        function calculateCombinedPalletConfiguration(combinedCartons, palletSize) {
            console.log(`\n🔧 ${palletSize.name}パレットで結合配置計算（高さ制限: ${maxHeightLimit}cm）`);
            
            const itemGroups = {};
            combinedCartons.forEach(carton => {
                if (!itemGroups[carton.code]) {
                    itemGroups[carton.code] = {
                        code: carton.code,
                ...
                itemGroups[carton.code].remaining++;
            });
            
            const availableItems = Object.values(itemGroups).filter(item => item.h <= getMaxCartonHeight());
            console.log(`高さ制限内アイテム: ${availableItems.length}/${Object.values(itemGroups).length}種類`);
            
            if (availableItems.length === 0) {
                console.log('高さ制限により配置可能なアイテムなし');
                return null;
            }
            
            const sortedItems = availableItems.sort((a, b) => {
                const aVolume = a.l * a.w * a.h;
                const bVolume = b.l * b.w * b.h;
                return bVolume - aVolume;
            });
            
            const result = calculatePalletConfigurationForItem(sortedItems, palletSize, sortedItems[0]);
            
            if (result && result.height > maxHeightLimit) {
                console.log(`❌ 結果が高さ制限超過: ${result.height.toFixed(1)}cm > ${maxHeightLimit}cm`);
                return null;
            }
        ...
        function autoOptimizePallets() {
            ...
            console.log(`\n=== 自動パレット最適化開始（高さ制限: ${maxHeightLimit}cm） ===`);
            ...
            for (let i = 0; i < window.currentPallets.length; i++) {
                for (let j = i + 1; j < window.currentPallets.length; j++) {
                    ...
                    const lowUtilization = pallet1.cartons.length <= 8 || pallet2.cartons.length <= 8;
                    const heightOk = maxHeight <= (maxHeightLimit * 0.8); // 余裕を持って
                    const weightOk = totalWeight <= 800;
                    const sizeReasonable = totalCartons <= 40;
                    
                    if (lowUtilization && heightOk && weightOk && sizeReasonable) {
                        const efficiency = (pallet1.cartons.length + pallet2.cartons.length) / 2;
            ...
            if (optimizationCandidates.length === 0) {
                alert(`📊 最適化候補が見つかりませんでした。\n\n高さ制限${maxHeightLimit}cm以内で結合可能なパレットペアがありません。\n現在のパレット配置は既に効率的です。`);
                return;
            }
            
            optimizationCandidates.sort((a, b) => a.efficiency - b.efficiency);
            const bestCandidate = optimizationCandidates[0];
            
            const confirmMessage = `🔍 自動最適化候補を発見しました！\n\n` +
                `📦 パレット${bestCandidate.index1 + 1}: ${window.currentPallets[bestCandidate.index1].cartons.length}個\n` +
                `📦 パレット${bestCandidate.index2 + 1}: ${window.currentPallets[bestCandidate.index2].cartons.length}個\n\n` +
                `🔄 結合後予想: ${bestCandidate.totalCartons}個\n` +
                `📏 高さ制限: ${maxHeightLimit}cm以内\n` +
                `💡 パレット数: 2枚 → 1枚 (50%削減)\n\n` +
                `この最適化を実行しますか？`;
            
            if (confirm(confirmMessage)) {
                document.getElementById('pallet1Select').value = bestCandidate.index1;
                document.getElementById('pallet2Select').value = bestCandidate.index2;
        ...
        function analyzeSelectedPallets() {
            ...
            console.log(`\n=== 詳細パレット結合分析（高さ制限: ${maxHeightLimit}cm） ===`);
            
            let analysisMessage = `🔍 パレット${pallet1Index + 1}とパレット${pallet2Index + 1}の結合分析\n`;
            analysisMessage += `📏 高さ制限: ${maxHeightLimit}cm\n\n`;
            
            analysisMessage += `📊 基本情報:\n`;
            analysisMessage += `  パレット${pallet1Index + 1}: ${pallet1.cartons.length}個, ${pallet1.height.toFixed(1)}cm\n`;
            analysisMessage += `  パレット${pallet2Index + 1}: ${pallet2.cartons.length}個, ${pallet2.height.toFixed(1)}cm\n`;
            analysisMessage += `  結合後: ${combinedCartons.length}個\n\n`;
            ...
            selectedPalletSizes.forEach(palletSize => {
                ...
                analysisMessage += `📐 ${palletSize.name}パレット:\n`;
                
                if (testPallet) {
                    const placedCount = testPallet.cartons.length;
                    const efficiency = (placedCount / combinedCartons.length) * 100;
                    const heightCompliant = testPallet.height <= maxHeightLimit;
                    
                    analysisMessage += `  実際配置: ${placedCount}個 (${efficiency.toFixed(1)}%)\n`;
                    analysisMessage += `  実際高さ: ${testPallet.height.toFixed(1)}cm\n`;
                    analysisMessage += `  高さ制限: ${heightCompliant ? '✅ 適合' : '❌ 超過'} (制限${maxHeightLimit}cm)\n`;
                    
                    if (heightCompliant) {
                        bestSolutions.push({
                            palletSize: palletSize.name,
                            placedCount,
                            efficiency,
                            height: testPallet.height,
                            compliant: true
                        });
                    }
                } else {
                    analysisMessage += `  配置結果: 失敗（高さ制限超過または配置不可）\n`;
                }
            ...
            if (bestSolutions.length > 0) {
                const bestSolution = bestSolutions.sort((a, b) => b.efficiency - a.efficiency)[0];
                analysisMessage += `🎯 推奨解決案:\n`;
                analysisMessage += `  最適: ${bestSolution.palletSize}パレット\n`;
                analysisMessage += `  配置効率: ${bestSolution.efficiency.toFixed(1)}%\n`;
                analysisMessage += `  高さ: ${bestSolution.height.toFixed(1)}cm ≤ 制限${maxHeightLimit}cm\n`;
                analysisMessage += `  ✅ 高さ制限適合\n`;
            } else {
                analysisMessage += `❌ 高さ制限${maxHeightLimit}cm以内での結合不可能\n`;
                analysisMessage += `💡 代替案:\n`;
                analysisMessage += `  • 高さ制限を緩和\n`;
                analysisMessage += `  • より大きなパレットサイズを追加\n`;
                analysisMessage += `  • 別のパレットペアを検討\n`;
            }
        ...
        function buildSummaryTable(pallets) {
            ...
            pallets.forEach((pallet, palletIndex) => {
                ...
                const dimensions = `${pallet.palletSize.width}×${pallet.palletSize.depth}×${pallet.height.toFixed(1)}`;
                ...
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td><span class="pallet-link" onclick="scrollToPallet(${palletIndex})">${palletIndex + 1} ${heightStatus}</span></td>
                    <td>${dimensions}</td>
                    <td>${weight}</td>
                    <td>${codes}</td>
                    <td>${quantities}</td>
